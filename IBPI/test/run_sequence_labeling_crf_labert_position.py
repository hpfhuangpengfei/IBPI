...
flags.DEFINE_bool("do_adversarial_train", True, "对抗.")
...
class InputExample(object):
  ...
  def __init__(self, guid, words, labels=None,ss=None,ee=None):
    self.guid = guid
    self.words = words
    self.labels = labels
    self.ss = ss
...
class SequenceLabelingProcessor(DataProcessor):
  ...
  def get_raw_data(input_file):
    ...
    with tf.io.gfile.GFile(input_file, 'r') as f:
      ...
      for line in f:
        ...
        if len(line) == 0:
          if len(words) > 0:
            examples.append((words, labels,ss,ee))
          words, labels, ss, ee = [], [], [], [] 
        else:
          # fields = line.split()
          ...
          word, label,ss_,ee_ = fields[0], fields[1] , fields[2] , fields[3]
  ...
  def _create_examples(self, raw_data, set_type):
    examples = []
    for (i, (words, labels,ss,ee)) in enumerate(raw_data):
      guid = "%s-%s" % (set_type, i)
      if set_type == "test" or set_type == "unlabeled":
        labels = None
      examples.append(InputExample(guid=guid, words=words, labels=labels,ss=ss,ee=ee))
    return examples
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  if isinstance(example, PaddingInputExample):
    return InputFeatures(
        ...
        input_s=[0.] * max_seq_length)

  label_map = {}
  for (i, label) in enumerate(label_list):
    label_map[label] = i

  segment_ids, label_positions, label_ids, label_weights,input_s = [], [], [], [], []
  ...
  input_s = [int(i) + int(j) for i,j in zip(example.ss,example.ee)]
  ...
  if len(input_ids) > max_seq_length:
    ...
    input_s = input_s[:max_seq_length]
  ...
  while len(input_s) < max_seq_length:
    input_s.append(0)
  ...
  assert len(input_s) == max_seq_length
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    if feature is not None:
      ...
      features["input_s"] = create_int_feature(feature.input_s)
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  name_to_features = {
      ...
      "input_s": tf.io.FixedLenFeature([seq_length], tf.int64),
  }
...
class Position_Encoder:
    def __init__(self, input_s):
      # 使用 get_variable() 函数初始化 embedding，使用截断正态分布初始化权重值
      # 定义输入张量占位符
      # 创建一个 Embedding 层，其中 input_dim 表示词典大小，output_dim 表示 embedding 的维度
      self.embedding_layer = tf.keras.layers.Embedding(input_dim=10, output_dim=512)
      # self.linear = tf.keras.layers.Linear(512, 512) 测试效果不好注释
      self.input_s = input_s
    def get_position_emd(self):
      print('######################')
      print(self.input_s.shape)
      print('######################')
      position_inf = self.embedding_layer(self.input_s)  
      # position_inf = self.linear(position_inf)
      position_inf = tf.reshape(position_inf, shape=(-1, 512))
...
def create_model(labert_config, is_training, input_ids, input_mask, segment_ids,
                 ...
                 label_positions, num_labels,inputs_s,
  ...
  model2 = Position_Encoder(inputs_s)
  
  output_layer = model.get_sequence_output()
  position_inf = model2.get_position_emd()
  ...
  with tf.compat.v1.variable_scope("loss"):
    if is_training:
      # I.e., 0.1 dropout
      output_layer = tf.math.add(output_layer, position_inf)
      output_layer = tf.nn.dropout(output_layer, rate=0.1)
      print(output_layer.shape)
      print(position_inf.shape)
      print('#######################')

      # output_layer = tf.concat([output_layer, position_inf], 1)
...
def model_fn_builder(labert_config, num_labels, init_checkpoint,
  ...
  def _model_fn(features, labels, mode, params):
    ...
    payload = create_model(
      ...
      positional_embeddings_start=positional_embeddings_start,
      positional_embeddings_end=positional_embeddings_end,
      label_positions=label_positions, num_labels=num_labels,inputs_s=input_s,
