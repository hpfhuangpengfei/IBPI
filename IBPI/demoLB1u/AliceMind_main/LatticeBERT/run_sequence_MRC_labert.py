...
TRAIN_FILE_NAME = "train_data_BIO.txt"
DEV_FILE_NAME = "dev_data_BIO.txt"
...
flags.DEFINE_integer("label_embeddings_size", 200, "label embedding size.")

flags.DEFINE_string("test_file", None, "predict file name with path")
flags.DEFINE_string("out_file", None, "predict file name with path")
...
class InputExample(object):
  ...
  def __init__(self, guid, words, labels=None, ners=None):
    self.guid = guid
    self.words = words
    self.labels = labels
    self.ners = ners
...
def compute_adv_loss(embedding_output, labert_config, input_ids, input_mask,
  ...
  hidden_size = adv_output_layer.shape[-1]
...
class SequenceLabelingProcessor(DataProcessor):
  ...
  def get_raw_data(input_file):
    ...
    with tf.io.gfile.GFile(input_file, 'r') as f:
      words, labels, ner_types = [], [], []
      for line in f:
        line = line.strip()
        if len(line) == 0:
          if len(words) > 0:
            examples.append((words, labels, ner_types))
          words, labels, ner_types = [], [], []
        else:
          # fields = line.split()
          ...
          word, ner, label = fields[0], fields[1], fields[-1]
          word = tokenization.convert_to_unicode(word)
          label = tokenization.convert_to_unicode(label)
          ner = tokenization.convert_to_unicode(ner)
          words.append(word)
          labels.append(label)
          ner_types.append(ner)
  ...
  def _create_examples(self, raw_data, set_type):
    examples = []
    for (i, (words, labels, ners)) in enumerate(raw_data):
      guid = "%s-%s" % (set_type, i)
      if set_type == "test" or set_type == "unlabeled":
        labels = None
      examples.append(InputExample(guid=guid, words=words, labels=labels, ners=ners))
    return examples
...
label_ners = {
    'attribute_attribute_des': 0,
    'concept': 1,
    'entity_core_entity': 2,
    'event': 3,
    'topic': 4
  }
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  for i, word in enumerate(example.words):
    ...
    if ner_ids == -1:
      ner_ids = label_ners[example.ners[i]]
  ...
  if ex_index < 2:
    ...
    tf.compat.v1.logging.info("ner_ids: %s" % str(ner_ids))
...
def file_based_convert_examples_to_features(
  ...
  for (ex_index, example) in enumerate(examples):
    ...
    if feature is not None:
      ...
      features["ner_ids"] = create_int_feature([feature.ner_ids])
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
  ...
  name_to_features = {
      ...
      "ner_ids": tf.io.FixedLenFeature([], tf.int64),
  }
...
def create_model(labert_config, is_training, input_ids, input_mask, segment_ids,
                 ...
                 label_positions, num_labels, ner_ids,
  ...
  label_embeddings = tf.compat.v1.get_variable(
      "label_embeddings",
      shape=[len(label_ners), hidden_size],
      initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.02))

  with tf.compat.v1.variable_scope("loss"):


    label_encoder = tf.nn.embedding_lookup(label_embeddings, ner_ids)

    # Attention
    att_score = tf.matmul(output_layer, label_encoder, transpose_b=True)
    info = tf.matmul(att_score, label_encoder)
    
    # print('ccccccccc',output_layer)
    # print('ccccccccc',ner_ids)
    # print('ccccccccc',label_encoder)
    # print('ccccccccc',att_score)
    # print('ccccccccc',info)
    
    output_layer = tf.add(output_layer, info)
    
    # print(session.run(info))
...
def model_fn_builder(labert_config, num_labels, init_checkpoint,
                     beta1, beta2, epsilon,label_embeddings_size,
  ...
  def _model_fn(features, labels, mode, params):
    ...
    total_loss, _ = crf.crf_log_likelihood(seq_logits, label_ids, sequence_lengths,
                                                      transition_matrix)
    total_loss = tf.reduce_mean(-total_loss)

    predictions, predicted_scores = crf.crf_decode(seq_logits, transition_matrix, sequence_lengths)
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      logging_hook = LossLoggingHook(FLAGS.train_batch_size, every_n_iter=int(num_train_steps / 200 + 1))
...
#           scaffold_fn=None)
      ...
      eval_metrics = metric_fn(total_loss, label_ids, predictions, label_weights)   
...
def main(_):
  ...
  dist_strategy = tf.compat.v1.distribute.MirroredStrategy(
        devices=["/gpu:{}".format(str(i)) for i in range(FLAGS.n_gpus)],
  ...
  run_config = tf.compat.v1.estimator.RunConfig(
        train_distribute=dist_strategy if FLAGS.n_gpus > 1 else None,
        # eval_distribute=dist_strategy if FLAGS.n_gpus > 1 else None,
  ...
  model_fn = model_fn_builder(
    ...
    label_embeddings_size=FLAGS.label_embeddings_size,
